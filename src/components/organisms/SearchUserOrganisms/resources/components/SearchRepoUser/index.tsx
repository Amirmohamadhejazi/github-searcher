...
import { getUserRepos } from '@core/services/api/get-user-repos-api'
import { type TCriticalAnyType } from '@core/types/common/critical-any'

import { tikeImg } from '@public/images'

import calculator from './resources/utils/calculator'

const SearchRepoUser = ({ inputSearch, dataUser }: TCriticalAnyType) => {
    ...
    if (isSuccess) {
        const { convertRepData } = calculator(data?.data)
        ...
        return (
            <div className='flex flex-col gap-y-5 m-1'>
                <div className='flex gap-2 justify-between items-center flex-wrap'>
                    ...
                    <div className='flex items-center flex-wrap-reverse gap-2'>
                        <Select
                            ...
                            onChange={(e: TCriticalAnyType) => typeReposHandler(e)}
                            value={reposType}
                            searchable
                        />
                        {page > 1 && (
                ...
                <div className='w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3'>
                    {convertRepData.map((itemsRepo: TCriticalAnyType) => (
                        <div
                                            ...
                                            title={
                                                <React.Fragment>
                                                    <div className='w-full flex-wrap flex items-center gap-1 '>
                                                        {itemsRepo.topics.map(
                                                            (itemsTopic: TCriticalAnyType, index: number) => (
                                                                <div
